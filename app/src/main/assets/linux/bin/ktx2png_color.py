#!/usr/bin/env python3
"""KTX -> Colored PNG (auto format detection)
R11_EAC → palette coloring; ETC2_RGBA → direct decode
Usage: python3 ktx2png_color.py <input> [output]"""

import struct, sys, os, json, subprocess, tempfile, zlib, bisect
...
for base in ['/home/pvr',os.path.expanduser('~/pvr')]:
    ...
    if os.path.exists(cli) and os.path.exists(lib): PVR_CLI=cli; PVR_LIB=lib; break

# ── KTX constants ──
KTX1_ID=bytes([0xAB,0x4B,0x54,0x58,0x20,0x31,0x31,0xBB,0x0D,0x0A,0x1A,0x0A])
GL_R11=0x9270; GL_SR11=0x9271
...
def apply_palette(gray):
    p=load_palette(); n=len(gray); rgba=bytearray(n*4)
    if p is None:
        for i in range(n): g=gray[i]; rgba[i*4]=g; rgba[i*4+1]=g; rgba[i*4+2]=g; rgba[i*4+3]=255
        return rgba
    lums=[x[0] for x in p]; cols=[(x[1],x[2],x[3],x[4]) for x in p]; min_lum=lums[0]
    for i in range(n):
        g=gray[i]
        if g<min_lum: continue
        idx=bisect.bisect_left(lums,g)
        if idx>=len(p): idx=len(p)-1
        r,gv,b,a=cols[idx]; rgba[i*4]=r; rgba[i*4+1]=gv; rgba[i*4+2]=b; rgba[i*4+3]=a
    return rgba
...
def convert(filepath):
    ...
    if info['gl_type']!=0:
        ...
        if gtype==0x1401:
            ...
            if gf==0x1908: return w,h,bytearray(info['pixel_data']),fn+' [direct]'
            if gf==0x1907:
                out=bytearray(w*h*4); px=info['pixel_data']
                for i in range(w*h): out[i*4:i*4+3]=px[i*3:i*3+3]; out[i*4+3]=255
                return w,h,out,fn+' [direct]'

    if PVR_CLI is None: raise RuntimeError("PVR not available")

    if is_r11:
        # Single-color: PVR decode → palette
        gray=decode_r11_pvr(filepath)
        rgba=apply_palette(gray)
        return w,h,bytes(rgba),fn+' [mono]'
    else:
        # Multi-color: PVR decode directly
...
if __name__=='__main__':
    ...
    if single:
        ...
        odir=out or os.path.join(os.path.dirname(inp) or '.','ColoredPNG')
        os.makedirs(odir,exist_ok=True)
        outputs=[os.path.join(odir,os.path.splitext(os.path.basename(f))[0]+'.png') for f in inputs]

    print(f"KTX->Color | {len(inputs)} files -> {odir}/\n")
    ok=fail=0
